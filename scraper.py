...
from apex_performance_client import send_bank_performance
...
USER_AGENT = "bank-annual-report-scraper/1.0"
FIELD_ORDER = [
...
FIELD_TO_KEY = {
    ...
    "Leverage Ratio (LR)": "leverage",
    "Non-Performing Loan Ratio (NPL)": "npl",
    "Provision Coverage Ratio (PCR)": "provision",
    ...
    "Cash to Deposit Ratio (CDR)": "cdr",
    "Credit Rating (CR)": "creditRating",
}

FIELD_TO_INDICATOR_CODE = {
    "Capital Adequacy Ratio (CRAR)": "CRAR",
...
RATING_TOKEN_RE = re.compile(
    r"(?<![A-Z0-9])(AAA|AA\s*[-]?\s*[123]|AA\s*[+-]|AA|A\s*[-]?\s*[123]|A\s*[+-]|A|BBB\s*[+-]|BBB|BB\s*[+-]|BB|B\s*[+-]|B|CCC|CC|C|D)(?![A-Z0-9])"
)
...
def iter_pages_ocr(pdf_path, max_pages, zoom=2.0):
...
def iter_pages_ocr_selected(pdf_path, page_numbers, zoom=2.0, cache=None, max_pages=None):
    pytesseract, image_module = load_ocr_modules()
    if not fitz or not pytesseract or image_module is None:
        raise RuntimeError("OCR dependencies are not available")
    if not page_numbers:
...
def iter_with_progress(pages, desc="Scanning pages", log_every=50):
...
def extract_fields(
    ...
    progress_desc = "OCR scan" if source == "ocr" else "Text scan"
    for page_num, text in iter_with_progress(pages, desc=progress_desc, log_every=50):
...
def push_performance_to_apex(result, period="Annual"):
    if not isinstance(result, dict):
        return
    bank_name = result.get("bank") or (result.get("summary_row") or {}).get("Bank Name")
    year = result.get("year")
    summary = result.get("summary_row") or {}
    developer_rows = result.get("developer_rows") or []
    ...
    for row in developer_rows:
        ...
        if feature and feature != "Total Score":
            source_index[feature] = row
    ...
    for field_name in FIELD_ORDER:
        indicator_code = FIELD_TO_INDICATOR_CODE.get(field_name)
        if not indicator_code:
            continue
        value = summary.get(summary_columns.get(field_name))
        if value in (None, ""):
            print(f"APEX performance skip {bank_name} {year} {indicator_code}: empty value")
            total_skipped += 1
            continue
        total_attempted += 1
        score = score_field_value(field_name, value)
        feature_name = display_names.get(field_name, field_name)
        source_row = source_index.get(feature_name, {})
        source_url = source_row.get("Source_URL") or ""
        source_type = source_row.get("Source_Type") or ""
        print(
            f"APEX sending {bank_name} {year} {indicator_code} "
            f"(value={value}, score={score}, period={period})"
        )
        try:
            response = send_bank_performance(
                bank_name=bank_name,
            ...
            print(
                f"APEX performance error {bank_name} {year} {indicator_code}: {exc}",
                file=sys.stderr,
            )
            continue
        if response.get("error"):
            print(
                f"APEX performance failed {bank_name} {year} {indicator_code}: {response.get('error')}",
                file=sys.stderr,
            )
            total_failed += 1
            continue
        status = response.get("status_code")
        mode = response.get("mode") or "form"
        body = response.get("response") or "(empty)"
        print(f"APEX performance {bank_name} {year} {indicator_code} [{mode}] -> {status}: {body}")
        if response.get("ok") and status in (200, 201):
            print(f"APEX sent {bank_name} {year} {indicator_code} -> {status}")
            total_sent += 1
        else:
            print(
                f"APEX performance rejected {bank_name} {year} {indicator_code} -> {status}",
                file=sys.stderr,
            )
            total_failed += 1

    print(
        f"APEX performance summary {bank_name} {year}: "
        f"inserted={total_sent}/{total_attempted}, failed={total_failed}, skipped={total_skipped}"
    )
    if total_attempted > 0 and total_sent == total_attempted and total_failed == 0:
        print(f"APEX performance SUCCESS {bank_name} {year}: all attempted indicators inserted.")
    elif total_attempted == 0:
        print(f"APEX performance WARNING {bank_name} {year}: no indicators attempted.")
    else:
        print(
            f"APEX performance PARTIAL {bank_name} {year}: "
            f"some indicators were not inserted.",
            file=sys.stderr,
        )
...
def build_eligible_output_name(year):
    year_part = sanitize_filename(year or "UnknownYear")
    return f"Eligible_Bank_{year_part}.xlsx"
...
def write_year_outputs(results, year, output_dir, summary_output=None):
    ...
    # Preserve prior run results by merging with existing year files and replacing by bank key.
    if os.path.exists(summary_path):
        try:
            prev_summary_df = pd.read_excel(summary_path)
            if "Bank Name" in prev_summary_df.columns:
                summary_df = pd.concat([prev_summary_df, summary_df], ignore_index=True)
                summary_df = summary_df.drop_duplicates(subset=["Bank Name"], keep="last")
        except Exception:
            pass

    if os.path.exists(developer_path):
        try:
            prev_developer_df = pd.read_excel(developer_path)
            dedupe_keys = [col for col in ("Bank Name", "Year", "Feature") if col in prev_developer_df.columns]
            developer_df = pd.concat([prev_developer_df, developer_df], ignore_index=True)
            if dedupe_keys:
                developer_df = developer_df.drop_duplicates(subset=dedupe_keys, keep="last")
        except Exception:
            pass
...
def run_single_with_config(
    ...
    print(f"=== Scan START: {output_bank} {year} ===")
    print(f"Scanning PDF: {final_pdf_path}")

    def scan_pdf_for_fields(fields_to_scan):
        print(f"[{output_bank} {year}] TEXT scan START ({len(fields_to_scan)} fields)")
        keyword_hits = {}
        page_text_cache = {}
        ...
        print(f"[{output_bank} {year}] TEXT scan END (matches={len(raw_matches)})")
        if not raw_matches and pdfplumber is not None and fitz is not None:
            print(f"[{output_bank} {year}] No matches found. Retrying TEXT scan with alternate parser.")
            keyword_hits = {}
            page_text_cache = {}
            pages = iter_pages_pymupdf(final_pdf_path, args.max_pages)
            print(f"[{output_bank} {year}] TEXT scan (alternate) START")
            raw_matches = extract_fields(
                pages,
                fields_to_scan,
                scan_keywords=args.scan_keywords,
            ...
            print(f"[{output_bank} {year}] TEXT scan (alternate) END (matches={len(raw_matches)})")
    ...
    if ocr_field_names and ocr_pages and fitz is not None and ocr_available():
        ...
        try:
            print(f"[{output_bank} {year}] OCR scan START (pages={len(ocr_pages)}, fields={len(ocr_field_names)})")
            ...
            print(f"[{output_bank} {year}] OCR scan END (matches={len(ocr_matches)})")
        except Exception as exc:
    ...
    missing_fallback = [
        name
        for name in ("Non-Performing Loan Ratio (NPL)", "Credit Rating (CR)")
    ...
    if missing_fallback and fitz is not None and ocr_available():
        ...
        if low_text_pages:
            ...
            try:
                print(f"[{output_bank} {year}] OCR fallback START (pages={len(low_text_pages)})")
                ...
                print(f"[{output_bank} {year}] OCR fallback END (matches={len(ocr_matches)})")
            except Exception as exc:
                print(
    ...
    for field_name in FIELD_ORDER:
        ...
        if score is not None:
            total_score += score
    if eligibility_only:
        print(f"=== Scan END: {output_bank} {year} ===")
        return {
            "bank": output_bank,
            "year": year,
...
def emit_extraction_data(result):
    if not isinstance(result, dict):
        return
    bank_name = result.get("bank") or (result.get("summary_row") or {}).get("Bank Name")
    summary = result.get("summary_row") or {}
    ...
    if pdf_path:
        try:
            ...
            if rel and not rel.startswith(".."):
                local_file_url = f"/{rel}"
        except Exception:
            local_file_url = None

    def build_viewer_url(page, value):
        file_url = local_file_url or pdf_url
        if not file_url:
            return None
        fragment = []
        if page not in (None, ""):
            fragment.append(f"page={quote(str(page))}")
        if value not in (None, ""):
            fragment.append(f"search={quote(str(value))}")
        viewer = f"/pdfjs/web/viewer.html?file={quote(str(file_url))}"
        if fragment:
            viewer = f"{viewer}#{'&'.join(fragment)}"
        return viewer

    indicators = {}
    for field_name in FIELD_ORDER:
        key = FIELD_TO_KEY.get(field_name)
        if not key:
            continue
        column = summary_columns.get(field_name)
        value = safe_json_value(summary.get(column))
        score = safe_json_value(score_field_value(field_name, value))
        feature_name = display_names.get(field_name, field_name)
        feature_row = feature_index.get(feature_name, {})
        page = safe_json_value(feature_row.get("Page"))
        indicators[key] = {
            "value": value,
            "score": score,
            "page": page,
            "sourceUrl": build_viewer_url(page, value) or feature_row.get("Source_URL"),
        }

    payload = {
        "key": slugify_name(bank_name),
        "name": str(bank_name or "Unknown"),
        "totalScore": safe_json_value(summary.get("Total Score")) or 0,
        "indicators": indicators,
    }
    print(f"EXTRACTION_DATA {json.dumps(payload, ensure_ascii=False)}", flush=True)
...
def main():
    ...
    parser.add_argument(
        "--no-output",
        action="store_true",
        help="Skip writing XLSX output files and stream extraction records only",
    )
    parser.add_argument("--download-dir", default="downloads", help="PDF download directory")
    ...
    if args.pdf_url or args.pdf_path or args.bank:
        result = run_single(args.pdf_url, args.pdf_path, args.bank, args.year)
        ...
        if not args.no_output:
            ...
            write_year_outputs([result], args.year, args.output_dir, summary_output=output_path)
        return 0
    ...
    if not args.no_output:
        for year, results in results_by_year.items():
            write_year_outputs(results, year, args.output_dir)
    return 0
